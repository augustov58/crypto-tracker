...
export function TokenTable() {
  ...
  // Split tokens into visible (has price) and hidden (no price = likely spam)
  const { visibleTokens, hiddenTokens } = useMemo(() => {
    const visible: PortfolioToken[] = [];
    const hidden: PortfolioToken[] = [];

    for (const token of tokens) {
      if (token.price !== null && token.price > 0) {
        visible.push(token);
      } else {
        hidden.push(token);
      }
    ...
    // Sort visible by value descending
    visible.sort((a, b) => (b.usdValue ?? 0) - (a.usdValue ?? 0));
    // Sort hidden by balance descending
    hidden.sort((a, b) => b.totalBalance - a.totalBalance);
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            No tokens found. Add wallets and run a snapshot to see your
            holdings.
          </div>
        </CardContent>
      </Card>
  ...
  const renderTokenRow = (token: PortfolioToken, isHidden = false) => (
    <Fragment key={token.tokenId}>
      <TableRow
        className={`cursor-pointer hover:bg-accent/50 ${isHidden ? "opacity-60" : ""}`}
      ...
      >
        ...
        <TableCell className="text-right">
          {token.unrealizedPnl !== undefined ? (
            ...
            <div className="text-muted-foreground text-xs">No cost basis</div>
          )}
        </TableCell>
      </TableRow>
          ...
          <Collapsible open={showHidden} onOpenChange={setShowHidden}>
            <CollapsibleTrigger asChild>
              ...
              >
                ...
                {showHidden ? "Hide" : "Show"} {hiddenTokens.length} unpriced
                token{hiddenTokens.length !== 1 ? "s" : ""}
                {showHidden ? (
                  <ChevronDown className="h-4 w-4" />
                ) : (
                  <ChevronRight className="h-4 w-4" />
            ...
            <CollapsibleContent>
              <div className="mt-2 border-t pt-4">
                <p className="text-xs text-muted-foreground mb-3 px-2">
                  These tokens have no price data on CoinGecko and may be spam,
                  airdrops, or unlisted tokens.
                </p>
                <Table className="min-w-[700px]">
                  <TableBody>
                    {hiddenTokens.map((token) => renderTokenRow(token, true))}
                  </TableBody>
                </Table>
              </div>
            </CollapsibleContent>
